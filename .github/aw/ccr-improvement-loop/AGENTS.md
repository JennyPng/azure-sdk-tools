...
- **`ccr-dashboard-pages.yml`** / **`ccr-dashboard-publish.yml`** — publish the
  static dashboard to GitHub Pages (`pages` is a one-shot manual deploy; `publish`
  is dormant/opt-in and commits new run JSON into `dashboard/data/`).
...
- **Pure ESM TypeScript, NO build step.** `.ts` files are run **directly by Node**
  via built-in type stripping. **GitHub Actions runs Node 24.** Requires Node ≥ 24.
...
- **Dashboard tests are separate** (Node's built-in runner, not vitest):
  ```bash
  node --test dashboard/js/aggregate.test.mjs
  ```
...
Any edit to `ccr-improvement-loop.md` **must** be followed by a recompile and a
commit of **both** the `.md` and the generated `.lock.yml`.

```bash
cd <repo-root>                            # NOT this subdir
...
- `actionlint` flags `copilot-requests` as an "unknown scope" — its scope list is
  **stale**; GitHub requires that permission. Non-fatal, ignore it.
...
- Pick a **workflow repo** to host the loop (the repo where these workflows run).
  A **personal, public** repo works well for trials; the pacer state branch is
  `ccr-pacer-state`, default branch `main`.
- Dispatch the pacer: `gh workflow run ccr-pacer.yml --repo <owner>/<workflow-repo>`
  (`dry_run` defaults false). Dry-run first to reconfirm admission counts.
- Inspect a run: `gh run view <id> --repo <owner>/<workflow-repo>`; watch:
  `gh run watch <id> --repo <owner>/<workflow-repo> --exit-status --interval 30`.
- Read the ledger:
  `gh api repos/<owner>/<workflow-repo>/contents/ledger/<file>.json?ref=ccr-pacer-state -q .content | base64 -d`.
